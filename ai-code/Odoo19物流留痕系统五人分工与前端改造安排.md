...
- 管理员后台界面改造
- 老板追溯查看界面改造
- 后台整体导航、一级/二级菜单与信息架构设计
- 留痕与追溯、运输与调度、仓储接入、业务单据接入、商品与基础资料、统计与看板、设置与规则的后台前端设计草案
- 搜索、列表、详情、时间线、证据查看、工作台、看板、配置页等页面设计
- 后台组件规范、页面规范、交互规范和 Odoo Web 落地边界设计

### 这个角色的核心目标

他不直接拥有业务主模型，但拥有“整个后台前端体系是否清晰可用”的最终责任。
...
- `stock` / `sale` / `purchase` / `fleet` / `base` / `web` 等被复用的 Odoo 原生模块

### 首先要做的工作

1. 写《Odoo物流后台前端总体设计总览》
2. 写《前端模块结构与菜单总表》
3. 写《系统页面关系总览》
4. 收口《管理员后台界面改造方案》和《老板追溯界面改造方案》
5. 补《运输与调度模块前端设计草案》《留痕与追溯模块前端设计草案》《异常中心前端设计草案》《工作台模块前端设计草案》
6. 补仓储管理、业务单据、基础资料、统计与看板、设置与规则的前端接入或草案设计
7. 统一页面字段、动作、搜索筛选、交互规范、接口消费清单和技术分层口径
...
- 核心阅读链和运输调度链的页面草案
- 仓储管理与业务单据入口设计
- 页面字段、动作、交互、搜索筛选、接口消费、技术分层等规范文档

### 这个角色最重要的交付

不是只把追溯页面“换个皮”，而是把整套 Odoo 物流后台的前端结构做顺：

- 让追溯链成为核心阅读主线
- 让运输调度、仓储、单据、基础资料、看板、设置这些模块都能自然接入
- 让管理员和老板在后台里“查、看、判、追溯、反查”都顺畅
