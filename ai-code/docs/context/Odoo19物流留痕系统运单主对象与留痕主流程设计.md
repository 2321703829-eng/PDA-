...
- 门店节点 `customer_line` 表示该运单号下的门店级阅读与履约节点
- 订单行 `order_line` 表示归属于门店节点的业务明细
- 货物行 `goods_line` 表示最底层货物事实
- 留痕事件表示一次装车、到店、签收、异常等现场事实
- 证据对象表示图片、备注、签名等材料

最关键的一句话是：

**正式留痕围绕运单号做闭环，图片业务阅读优先围绕 `customer_line`，不是围绕单个订单做闭环。**
...
flowchart TD
    ...
    B --> G[运单号 Waybill 2]
    B --> H[运单号 Waybill N]
    F --> I[门店节点 CustomerLine]
    G --> J[门店节点 CustomerLine]
    H --> K[门店节点 CustomerLine]
    I --> I1[订单行 OrderLine]
    J --> J1[订单行 OrderLine]
    K --> K1[订单行 OrderLine]
    I1 --> I2[货物行 GoodsLine]
