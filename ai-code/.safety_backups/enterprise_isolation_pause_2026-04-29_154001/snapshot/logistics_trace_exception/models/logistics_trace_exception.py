...
EXCEPTION_HANDLER_GROUP = "logistics_trace_exception.group_logistics_exception_handler"
TRACE_MANAGER_GROUP = "logistics_trace_evidence.group_logistics_trace_manager"
DISPATCH_MANAGER_GROUP = "logistics_dispatch.group_logistics_dispatch_manager"
...
class LogisticsTraceException(models.Model):
    ...
    def _can_handle_exception(self):
        user = self.env.user
        return self.env.su or user.has_group("base.group_system") or user.has_group(
            EXCEPTION_HANDLER_GROUP
        ) or user.has_group(TRACE_MANAGER_GROUP) or user.has_group(DISPATCH_MANAGER_GROUP)
    ...
    def write(self, vals):
        ...
        if target_state:
            if not self._can_handle_exception():
                raise ValidationError(_("Only exception handlers can change exception state."))
            ...
            raise ValidationError(_("closed_time is managed by exception state transitions."))
        elif not self._can_handle_exception() and any(
            field_name in vals for field_name in ("close_summary", "process_owner_id", "process_owner_name")
        ):
            raise ValidationError(
                _("Only exception handlers can update close summary or process owner fields.")
            )
