...
class LogisticsTraceEvidence(models.Model):
    ...
    matched_waybill_id = fields.Many2one(
        "logistics.dispatch.waybill",
        string="命中运单",
        index=True,
    )
    ...
    match_remark = fields.Text(string="命中备注")
    ...
    image_access_key = fields.Char(string="封面图访问键", index=True)
    preview_url = fields.Char(string="封面预览地址")
    full_url = fields.Char(string="封面原图地址")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            self._apply_link_defaults(vals)
        return super().create(vals_list)

    def write(self, vals):
        vals = dict(vals)
        if "trace_event_id" in vals or "matched_waybill_id" in vals or "matched_customer_line_id" in vals:
            self._apply_link_defaults(vals, existing_records=self)
    ...
    @api.model
    def _apply_link_defaults(self, vals, existing_records=None):
        trace_event_id = vals.get("trace_event_id")
        trace_event = self.env["logistics.trace.event"].browse(trace_event_id) if trace_event_id else False
        if not trace_event and existing_records:
            trace_event = existing_records[:1].trace_event_id

        matched_waybill = False
        if "matched_waybill_id" in vals:
            matched_waybill = self.env["logistics.dispatch.waybill"].browse(vals["matched_waybill_id"])
        elif trace_event and trace_event.waybill_id:
            matched_waybill = trace_event.waybill_id
            vals["matched_waybill_id"] = matched_waybill.id

        if "matched_customer_line_id" not in vals and matched_waybill:
            customer_line = matched_waybill.customer_line_ids.sorted(key=lambda rec: (rec.sequence, rec.id))[:1]
            vals["matched_customer_line_id"] = customer_line.id if customer_line else False

        if "match_status" not in vals:
            vals["match_status"] = "matched" if matched_waybill else "unmatched"
        if "link_status" not in vals:
            vals["link_status"] = "linked" if trace_event else "pending"
        if vals.get("link_status") == "linked":
            vals.setdefault("linked_at", fields.Datetime.now())
            vals.setdefault("linked_by", self.env.user.id)
