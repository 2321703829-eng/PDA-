...
class LogisticsWebImportController(http.Controller):
    ...
    def precheck_waybill_standard_import(self, **kwargs):
        ...
        status = 200 if data["can_confirm_import"] or data["errors"] else 200
        response_code = 0 if data["can_confirm_import"] or data["errors"] else 1
        response_message = _("预校验完成") if data["errors"] else _("预校验通过")
        return self._json_response(
            {
                "code": response_code,
