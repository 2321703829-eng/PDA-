...
export class LogisticsStatsCenterAction extends Component {
    ...
    get ui() {
        return {
            ...
            waybillTrendHint: "观察按日、按周或按月聚合后的执行波动。",
        ...
        return [
            {
                ...
                caption: "观察当前窗口执行规模",
                tone: "info",
                value: this.state.overview.summary.waybill_count,
                percent: false,
            },
            {
                key: "signed_rate",
                ...
                caption: "判断整体履约质量",
                tone: "success",
                value: this.state.overview.summary.signed_rate,
                percent: true,
            },
            {
                key: "timeout_rate",
                ...
                caption: "定位当前时效风险",
                tone: "warning",
                value: this.state.overview.summary.timeout_rate,
                percent: true,
            },
            {
                key: "exception_count",
                ...
                caption: "追踪异常热度变化",
                tone: "danger",
                value: this.state.overview.summary.exception_count,
                percent: false,
            },
        ];
    }
    ...
    isCompactTrendSeries(series) {
        const points = series?.points || [];
        return points.length > 0 && points.length <= 4;
    }

    isCompactDataset(dataset) {
        const items = dataset?.items || [];
        return items.length > 0 && items.length <= 2;
    }
    ...
    getSeriesPointValue(series, bucket) {
        return Number(series?.points?.find((item) => item.bucket === bucket)?.value || 0);
    }
    getSeriesPointWidth(series, bucket) {
        const max = this.getMaxTrendValue(series?.points || []);
        const value = this.getSeriesPointValue(series, bucket);
        return max ? Math.max(14, Math.round((value / max) * 100)) : 14;
    }
