...
class LogisticsEvidenceImageStorage:
    ...
    def _get_absolute_path(self, evidence_record):
        relative_path = (evidence_record.storage_relative_path or "").strip()
    ...
    def sync_evidence_meta(self, evidence_record):
        absolute_path = self._get_absolute_path(evidence_record)
        if not absolute_path.exists():
            return {"state": "missing"}

        content_type = evidence_record.mime_type or mimetypes.guess_type(
            evidence_record.source_filename or evidence_record.stored_file_name or ""
        )[0]
        return {
            "source_filename": evidence_record.source_filename or evidence_record.stored_file_name,
            "stored_file_name": evidence_record.stored_file_name or absolute_path.name,
            "file_ext": evidence_record.file_ext or absolute_path.suffix.lower(),
            "mime_type": content_type,
            "file_size": absolute_path.stat().st_size,
            "storage_provider": evidence_record.storage_provider or "local",
            "storage_bucket": False,
            "storage_relative_path": evidence_record.storage_relative_path,
            "state": "available",
        }

    def read_image(self, evidence_record):
        absolute_path = self._get_absolute_path(evidence_record)
        if not absolute_path.exists():
            evidence_record.sudo().write({"state": "missing"})
        ...
        return {
            "file_name": evidence_record.source_filename or evidence_record.stored_file_name or absolute_path.name,
