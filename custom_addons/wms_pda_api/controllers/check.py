...
class WmsPdaCheckController(WmsPdaBaseController):
    ...
    def list_check_tasks(self, **kwargs):
        ...
        return self._handle_request(lambda user, wh, token: self._check_list_tasks(user, wh, payload))
    ...
    def get_check_task_lines(self, task_id, **kwargs):
        return self._handle_request(lambda user, wh, token: self._check_get_lines(user, wh, task_id, lock=True))
    ...
    def confirm_check_line(self, task_id, **kwargs):
        payload = self._get_payload()
        return self._handle_idempotent_request(payload, lambda user, wh, token: self._check_confirm_line(user, wh, task_id, payload))
    ...
    def complete_check_task(self, task_id, **kwargs):
        payload = self._get_payload()
        return self._handle_idempotent_request(payload, lambda user, wh, token: self._check_complete_task(user, wh, task_id, payload))
    ...
    def get_check_summary(self, task_id, **kwargs):
        return self._handle_request(lambda user, wh, token: self._check_summary_response(user, wh, task_id))

    def _check_list_tasks(self, user, warehouse, payload):
        if not warehouse:
        ...
        return {"total": total, "records": [self._check_format_task(task) for task in tasks]}

    def _check_get_lines(self, user, warehouse, task_id, lock=False):
        task = self._check_get_task(user, warehouse, task_id)
        ...
        lines = self._check_ensure_check_lines(task)
        return {
            "task": self._check_format_task(task, include_lock=True),
            "lines": [self._check_format_line(line, idx + 1) for idx, line in enumerate(lines)],
            "progress": self._check_progress(task),
        }

    def _check_confirm_line(self, user, warehouse, task_id, payload):
        task = self._check_get_task(user, warehouse, task_id)
        ...
        line = self._check_get_check_line(task, payload)
        barcode = (payload.get("product_barcode") or payload.get("barcode") or "").strip()
        checked_qty = self._check_float_payload(payload, "checked_qty", fallback_key="done_qty")
        if checked_qty <= 0:
            raise ValidationError("复核数量必须大于 0。")
        if barcode and not self._check_product_matches_barcode(line.product_id, barcode):
            return self._error(self.ERR_BARCODE_UNKNOWN, "扫描商品与当前复核行不一致。", status=400, tts="商品不匹配")
        state = self._check_line_state(checked_qty, line.picked_qty)
        line.write({"checked_qty": checked_qty, "state": state})
        next_line = self._check_next_line(task)
        return self._success(
            {
                ...
                "status": state,
                "next_line": self._check_format_next_line(next_line) if next_line else {},
            },
            tts=self._check_check_tts(line, next_line),
        )

    def _check_complete_task(self, user, warehouse, task_id, payload):
        task = self._check_get_task(user, warehouse, task_id)
        if task.state == "checked":
            return {"task_state": task.state, "summary": self._check_completion_summary(task), "next_step": "handover"}
        if task.state not in ("waiting_check", "checking"):
            return self._error(self.ERR_STATE_CONFLICT, "当前状态不能完成复核。", status=400, tts="状态不允许")
        self._check_ensure_check_lines(task)
        task.action_mark_checked()
        self._check_save_photos(task, user, warehouse, payload)
        request.env["wms.task.lock"].sudo().release(task._name, task.id, user_id=user.id)
        return self._success(
            {
                "task_state": task.state,
                "summary": self._check_completion_summary(task),
    ...
    def _check_summary_response(self, user, warehouse, task_id):
        task = self._check_get_task(user, warehouse, task_id)
        self._check_ensure_check_lines(task)
        return {"task_id": task.id, "state": task.state, "progress": self._check_progress(task), "exceptions": []}

    def _check_get_task(self, user, warehouse, task_id):
        domain = [("id", "=", task_id)]
        if warehouse:
    ...
    def _check_ensure_check_lines(self, task):
    ...
    def _check_get_check_line(self, task, payload):
        self._check_ensure_check_lines(task)
        ...
        line = lines.filtered(lambda item: self._check_product_matches_barcode(item.product_id, barcode))[:1]
        if not line:
            raise ValidationError("未找到匹配的复核明细。")
        return line

    def _check_format_task(self, task, include_lock=False):
    ...
    def _check_format_next_line(self, line):
        return {
            "line_id": line.id,
            "product_name": line.product_id.display_name,
            "picked_qty": line.picked_qty,
    ...
    def _check_next_line(self, task):
    ...
    def _check_completion_summary(self, task):
        lines = self._check_ensure_check_lines(task)
        return {
            "total_lines": len(lines),
            "total_checked_qty": sum(lines.mapped("checked_qty")),
    ...
    def _check_save_photos(self, task, user, warehouse, payload):
    ...
    def _check_float_payload(self, payload, key, fallback_key=False):
    ...
    def _check_line_state(self, checked_qty, picked_qty):
    ...
    def _check_product_matches_barcode(self, product, barcode):
        return barcode in {product.barcode, product.default_code, product.product_tmpl_id.default_code}

    def _check_check_tts(self, line, next_line):
