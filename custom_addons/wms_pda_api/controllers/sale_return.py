...
class WmsPdaSaleReturnController(WmsPdaBaseController):
    ...
    def _list_tasks(self, user, warehouse, payload):
        ...
        return {
            ...
            "offset": offset,
            "limit": limit,
            "records": [self._format_task(task, include_progress=True) for task in tasks],
        }
    ...
    def _confirm_line(self, user, warehouse, task_id, payload):
        ...
        move = self._get_move(task, payload)
        ...
        if barcode and not self._product_matches_barcode(move.product_id, barcode):
        ...
        self._write_done_qty(move, done_qty)
        self._write_quality_state(move, quality_state)
        remaining_lines = len([item for item in self._task_moves(task) if self._read_done_qty(item) <= 0])
        return self._success(
            {
                ...
                "task": self._format_task(task, include_progress=True),
            },
    ...
    def _complete_task(self, user, warehouse, task_id, payload):
        ...
        if task.state == "received":
            return {
                ...
                "recovery_status": self._recovery_status(task),
            }
        if task.state not in ("waiting_receipt", "receiving"):
            return self._error(self.ERR_STATE_CONFLICT, "当前状态不能完成退货入库。", status=400, tts="状态不允许")
        self._save_photos(task, payload.get("photo_urls") or [], user)
        task.action_mark_received()
        self._mark_sale_return_recovered(task)
    ...
    def _get_move(self, task, payload):
        ...
        matched = moves.filtered(lambda record: self._product_matches_barcode(record.product_id, barcode))[:1]
        if not matched:
    ...
    def _format_task(self, task, include_progress=False):
        picking = task.stock_picking_id
            ...
            result.update(
                {
                    ...
                    "done_line_count": len(moves.filtered(lambda move: self._read_done_qty(move) > 0)),
                    "demand_qty": sum(moves.mapped("product_uom_qty")),
                    "done_qty": sum(self._read_done_qty(move) for move in moves),
                    "lines": [self._format_move(move) for move in moves],
                }
            )
        return result
    ...
    def _format_move(self, move):
        ...
        return {
            ...
            "demand_qty": move.product_uom_qty or 0.0,
            "done_qty": self._read_done_qty(move),
            "quality_state": self._read_quality_state(move),
            "uom": move.product_uom.name if move.product_uom else "",
            "state": move.state,
        }

    def _mark_sale_return_recovered(self, task):
        picking = task.stock_picking_id
        if not picking:
            return
        vals = {}
    ...
    def _write_quality_state(self, move, quality_state):
    ...
    def _read_quality_state(self, move):
        if "quality_state" in move._fields:
