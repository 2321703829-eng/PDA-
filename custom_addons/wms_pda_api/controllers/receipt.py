...
class WmsPdaReceiptController(WmsPdaBaseController):
    ...
    def _list_tasks(self, user, warehouse, payload):
        ...
        return {
            ...
            "tasks": [self._format_task(task, include_progress=True) for task in tasks],
        }
    ...
    def _get_task_lines(self, user, warehouse, task_id, lock=False):
        ...
        return {
            ...
            "lines": [self._format_move(move) for move in self._task_moves(task)],
        }
    ...
    def _format_task(self, task, include_progress=False):
        ...
        if include_progress:
            moves = self._task_moves(task)
            done_moves = moves.filtered(lambda move: self._read_done_qty(move) > 0)
            result.update(
                {
