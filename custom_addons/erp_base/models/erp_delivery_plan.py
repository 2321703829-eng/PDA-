...
class ErpDeliveryPlan(models.Model):
    ...
    planned_date = fields.Date(string="计划出库日", default=fields.Date.context_today)
    state = fields.Selection(
        [("draft", "草稿"), ("confirmed", "已确认"), ("in_progress", "执行中"),
         ("done", "已完成"), ("cancelled", "已取消")],
        string="状态", default="draft",
    )
    picking_ids = fields.One2many("stock.picking", "origin", string="关联出库单")
