...
class TestBiSnapshot(TransactionCase):
    ...
    def test_01_create_snapshot(self):
        """创建 KPI 日报快照"""
        snap = self.env["bi.ops.kpi.snapshot"].create({
    ...
    def test_02_generate_snapshot_runs(self):
        ...
        if hasattr(snap, 'generate_snapshot'):
            ...
            found = self.env["bi.ops.kpi.snapshot"].search([
                ("snapshot_date", "=", date.today())
            ])
    ...
    def test_03_snapshot_date_unique_per_day(self):
        """同一日期可创建多条快照(不强制唯一)"""
        s1 = self.env["bi.ops.kpi.snapshot"].create({"snapshot_date": date.today()})
        s2 = self.env["bi.ops.kpi.snapshot"].create({"snapshot_date": date.today()})
...
class TestBiDashboardBoard(TransactionCase):
    ...
    def test_01_create_board(self):
        """仪表盘记录可创建"""
        board = self.env["bi.dashboard.board"].create({})
...
class TestBiSnapshotModels(TransactionCase):
    ...
    def test_01_order_snapshot(self):
        """订单快照模型存在"""
        self.assertTrue(hasattr(self.env, "bi.ops.kpi.snapshot"))
        # 确保模型可被搜索
        model = self.env["ir.model"].search([("model", "=", "bi.ops.kpi.snapshot")], limit=1)
        self.assertTrue(model)

    def test_02_dashboard_board_model_exists(self):
        """仪表盘模型存在"""
        model = self.env["ir.model"].search([("model", "=", "bi.dashboard.board")], limit=1)
        self.assertTrue(model)
